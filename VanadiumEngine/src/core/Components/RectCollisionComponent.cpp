...
namespace Vanadium
{
	...
	Vector2 RectCollisionComponent::EscapeVector(Vector2 point)
	{
		...
		for (u32 i = 0; i < 4; i++)
		{
			if (Math::Length2(escapeVectors[i]) < Math::Length2(escapeVectors[shortest]))
			{
				shortest = i;
			}
		}
	...
	Vector2 RectCollisionComponent::CollisionVertex(RectCollisionComponent* other)
	{
		...
		float v1Dist = Math::Length2(otherWorldCenter - v1);
		float v2Dist = Math::Length2(otherWorldCenter - v2);
		float v3Dist = Math::Length2(otherWorldCenter - v3);
		float v4Dist = Math::Length2(otherWorldCenter - v4);
